...
void CArchive::WriteObject(const CMyObject* pOb)
{
	// object can be NULL
	...
	else
	{
		...
		((CMyObject*)pOb)->Serialize(*this);
	}
}

CMyObject* CArchive::ReadObject(const CRuntimeClass* pClassRefRequested)
{
	...
	if (pClassRef == NULL)
	{
		...
		pOb = (CMyObject*)m_pLoadArray->GetAt(obTag);
...
void CArchive::MapObject(const CMyObject* pOb)
{
	if (IsStoring())
	{
		if (m_pStoreMap == NULL)
